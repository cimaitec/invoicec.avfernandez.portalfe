...
@ViewScoped
@ManagedBean
public class CabDocumentoDoc implements Serializable
{
	...
	private boolean flagInputCF = false;//Flag para desactivar Textbox de RUC/CEDULA/PASAPORTE
	...
	//TODO contructor para cuando se selecciona cnsumidor final
	public void seleccionCF(){
		if(consumidorf){
			setRuc("9999999999999");
			flagInputCF = true;//se desactiva caja de texto
		}else{
			setRuc("");
			flagInputCF = false;//se activa caja de texto
		}
	}
	...
	public LazyDataModel<FacCabDocumento> getAllDocumentos() {
		...
		if ( (ruc !=null) && (numDocumento !=null)){
			numDocumentoTmp = numDocumento.replace("-", "");
			if ((ruc.trim()).length()>=5 && numDocumentoTmp.trim().length()>14){
	...
	public void findDocumentos()
	{
		...
		if (ruc !=null){
			...
			mensajeAlerta("Mensaje del sistema","Debe ingresar la Identificación", "informacion");
			flagConsulta = false;
		}
		...
		if (numDocumento !=null){
			...
			mensajeAlerta("Mensaje del sistema","Debe ingresar el numero del documento", "informacion");
			flagConsulta = false;
		}
	...
	public void findConsultaDocVelidate()
	{
		boolean flagConsulta = true;
		if (ruc.equals("")){
			mensajeAlerta("Mensaje del sistema","Debe ingresar la Identificación", "informacion");
	...
	//TODO contructor de mensaje de alerta
	 private void mensajeAlerta(String mensajeVentana, String mensajeDetalle, String tipo) {
		 FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(mensajeDetalle));
	...
	public boolean isFlagInputCF() {
		return flagInputCF;
	}

	public void setFlagInputCF(boolean flagInputCF) {
		this.flagInputCF = flagInputCF;
	}
